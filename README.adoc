...
This repository uses a **branch-per-version** strategy to maintain documentation and code examples for different MicroProfile releases. Each major MicroProfile version has its own dedicated branch, ensuring clear separation and stability.

### Branch Organization

* **`main`** - Contains MicroProfile 6.1 documentation and code examples
* **`v7.1`** - Contains MicroProfile 7.1 documentation and code examples
* **Future versions** - Will follow the same pattern `v7.2`, `v8.0`, etc.
...
2. **Update version numbers** in:
   * `antora.yml` - Component version
   * All `pom.xml` files in `code/` directory - MicroProfile dependencies
...
5. **Push the branch** and create a pull request when ready

### Building Multiple Versions

The `antora-assembler.yml` file is configured to build documentation for multiple versions simultaneously, creating a version selector in the UI. This allows users to switch between different MicroProfile versions easily.
