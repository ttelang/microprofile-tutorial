...
* **CDI Integration** - Scope management (`@ApplicationScoped`, `@RequestScoped`), lifecycle management
...
* **RestClientBuilder API** - Programmatic client creation, complete configuration options (timeouts, SSL, proxies), when to use vs CDI injection
* **Custom Filters & Interceptors** - ClientRequestFilter/ClientResponseFilter for HTTP-level interception (authentication, logging, header manipulation), WriterInterceptor/ReaderInterceptor for entity-level processing (compression, encryption), three registration methods

** Code Examples**
 
* MP E-commerce servces (Product, Order, Payment services) in Chapter 11 have been updated to use MicroProfile Rest Client 4.0
* Updated Maven and Gradle configurations to use the latest MicroProfile dependencies
* New examples demonstrating the simplified `baseUri(String)` method
* Request/response body validation
* Complete CRUD operations combining all concepts
* **Custom filters** - ClientRequestFilter and ClientResponseFilter implementations that intercept HTTP requests/responses to add cross-cutting concerns like authentication, logging, and header manipulation without modifying business logic
* **Multipart handling** - Uploading and processing multipart/form-data for file uploads, combining binary content with metadata in a single request using JAX-RS multipart annotations
* **Error mapping** - ResponseExceptionMapper implementations that transform HTTP error responses into domain-specific exceptions, providing centralized error handling with priority-based mapping strategies (using `@Priority` annotations to control which mapper handles specific status codes first when multiple mappers are registered)
* **Authentication filters** - Practical implementations of Basic Authentication, Bearer Token (OAuth/JWT), and header propagation patterns that automatically inject credentials into outgoing REST client requests using ClientRequestFilter
* **Response/request logging and caching filters** - Filter implementations that log HTTP request/response details for debugging and auditing, plus client-side caching strategies using conditional requests (ETags, Last-Modified headers) to reduce network traffic and improve performance
* **GZIP compression/decompression interceptors** - WriterInterceptor and ReaderInterceptor implementations that automatically compress outgoing request bodies and decompress incoming response bodies using GZIP, reducing bandwidth usage for large payloads
