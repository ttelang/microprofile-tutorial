...
This developer guide offers step-by-step instructions for downloading, compiling, and running the MicroProfile e-commerce application source code that demonstrates benefits of MicroProfile features in real-world scenarios.

toc::[]

## Overview

The MicroProfile e-commerce store is a microservices-based application built with Jakarta EE and MicroProfile.
...
│   └── README.adoc             # Essential information
```
...
* **Maven 3.9 or later** - For building the applications
...
The tutorial applications use Maven and the link:https://github.com/OpenLiberty/ci.maven[Open Liberty Maven plugin] for building and running.
...
We use Derby embedded database for simplicity. The database configuration is in `src/main/resources/META-INF/persistence.xml`:
...
* **MicroProfile** - Cloud-native microservices specifications
...
* **MicroProfile GraphQL** - API for GraphQL endpoints
* **MicroProfile Reactive Messaging** - Asynchronous messaging based on Reactive Streams
...
- Verify JDK version is 21 or later
...
1. **Check the README**: Each project has its own README.adoc with specific information
...
4. **Integrate Services**: Practice calling one service from another using REST Client (covered in Chapter 11)
