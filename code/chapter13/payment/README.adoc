= Payment Service with Reactive Messaging

This example demonstrates MicroProfile Reactive Messaging 3.0.1 in the payment service used by Chapter 13.

== Overview

The payment service is adapted to demonstrate a REST-to-messaging flow. A payment request is accepted over HTTP, published to an internal channel, and then processed asynchronously by a reactive consumer.

This project showcases:

* **Programmatic message emission** using `@Channel` and `Emitter`
* **Reactive message consumption** with `@Incoming`
* **Manual acknowledgment** using `Message<T>`
* **Configuration-driven payment gateway integration** with MicroProfile Config
* **Supporting REST endpoints** for payment configuration and product integration
...
└── src/main/java/io/microprofile/tutorial/store/payment/
    ├── service/
    │   ├── PaymentService.java             # REST-to-messaging bridge and reactive consumer
    │   └── ProductIntegrationService.java  # Supporting payment/product integration logic
    ├── resource/
    │   ├── PaymentConfigResource.java      # Config management endpoints
    │   └── PaymentProductResource.java     # Product lookup examples for payment flows
    ├── entity/
    │   └── PaymentDetails.java             # Payment request payload
    └── config/
        ├── PaymentConfig.java              # Config access helper
        └── PaymentServiceConfigSource.java # Custom ConfigSource
└── src/main/resources/META-INF/
    └── microprofile-config.properties      # Payment and integration configuration
----

== Code Examples

=== 1. PaymentService - REST to Reactive Messaging Bridge

The payment service accepts an HTTP request and publishes the payload to an internal reactive channel.
...
=== 2. PaymentService - Asynchronous Consumer with Acknowledgment

The same service then consumes the internal channel asynchronously.
...
=== 3. PaymentConfigResource - Configuration Support

The example also keeps the MicroProfile Config support used by the payment service.
...
public Response getPaymentConfig() {
    return Response.ok(Map.of(
        "gateway.endpoint", PaymentConfig.getConfigProperty("payment.gateway.endpoint")
    )).build();
}
----
...
* Publishes a payment request to the internal `payment-requests` channel
...
* Response: `{"status":"accepted", "message":"Payment request accepted for asynchronous processing."}`
* Reactive flow:
  1. REST endpoint accepts the request
  2. `Emitter<PaymentDetails>` publishes it to `payment-requests`
  3. `@Incoming("payment-requests")` processes it asynchronously
  4. the message is acknowledged after the simulated gateway call succeeds
...
|Payment gateway endpoint URL used by the reactive payment consumer
...
* OpenAPI UI: `http://localhost:9050/payment/api/openapi-ui/`
* OpenAPI JSON: `http://localhost:9050/payment/api/openapi`
...
[source,xml]
----
<liberty.var.default.http.port>9050</liberty.var.default.http.port>
<liberty.var.default.https.port>9051</liberty.var.default.https.port>
----
