...
=== API & Web Services
* *RESTful CRUD Operations* - Complete Create, Read, Update, Delete operations for products
* *Advanced Search* - Query products by name, description, and price range using query parameters
* *Jakarta RESTful Web Services* - JAX-RS annotations (@GET, @POST, @PUT, @DELETE)
* *JSON Processing* - Automatic JSON serialization/deserialization with JSON-B

=== Data Persistence
* *Jakarta Persistence (JPA)* - Object-relational mapping with entity annotations
* *Named Queries* - Predefined JPQL queries for common operations
* *Derby Embedded Database* - Zero-configuration in-memory database
* *Multiple Repository Implementations* - Switchable JPA and In-Memory repositories using CDI qualifiers

=== Validation & Quality
* *Bean Validation* - Input validation with Jakarta Validation annotations (@NotNull, @NotBlank, @Size, @DecimalMin)
* *Entity Validation* - Automatic validation on Product entity fields

=== Logging & Monitoring
* *Java Util Logging* - Standard JDK logging framework (java.util.logging.Logger)
* *Manual Logging* - Explicit logging statements in REST resources and service layer
* *Operation Tracking* - Logs for all CRUD operations and search queries
* *Log Levels* - INFO for normal operations, structured logging with parameters

NOTE: This service uses manual logging. For automatic method logging with CDI interceptors, see the Payment Service in this chapter, which uses a `@Logged` annotation combined with a `LoggingInterceptor` class annotated with `@Interceptor` and `@AroundInvoke`. The interceptor automatically captures method entry, exit, parameters, return values, execution time, and exceptions without cluttering the business logic with logging code. This approach demonstrates aspect-oriented programming (AOP) where cross-cutting concerns like logging are separated from core business functionality.

=== Dependency Injection
* *CDI (Contexts and Dependency Injection)* - Loose coupling with @Inject and @ApplicationScoped
* *CDI Qualifiers* - Custom qualifiers (@JPA, @InMemory) for selecting repository implementations
* *Repository Pattern* - Interface-based data access layer with multiple implementations
