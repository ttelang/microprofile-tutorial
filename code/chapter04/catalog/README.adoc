...
The MicroProfile Catalog Service provides a RESTful API for product catalog management with flexible persistence options. It is built with MicroProfile 7.1 and Jakarta EE 10 specifications.

This project demonstrates key capabilities of MicroProfile OpenAPI, CDI qualifiers, and the dual persistence architecture with both JPA/Derby database and in-memory implementations.
...
* *Portability*: Suitable for containerized deployments
...
git clone https://github.com/ttelang/microprofile-tutorial.git
cd code/catalog
...
* *OpenAPI documentation not available*: Make sure `mp.openapi.scan.disable=true` is not set in the properties file
