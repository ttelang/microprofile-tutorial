...
The catalog service includes a logging interceptor that provides automatic method entry/exit logging with execution time tracking. It's implemented using CDI interceptors and merged from the mp-ecomm-store project.
...
The logging interceptor uses the standard Java logging framework. The `logging.properties` file in `src/main/resources` configures the logging level and handlers.
