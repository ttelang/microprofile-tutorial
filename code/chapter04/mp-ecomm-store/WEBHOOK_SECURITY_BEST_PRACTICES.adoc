...
This guide covers essential security practices for implementing webhook receivers that integrate with the MicroProfile OpenAPI webhook implementation.
...
Use JSON Schema pattern validation to enforce HTTPS:

[source, java]
----
@Schema(
    description = "Callback URL (must be HTTPS)",
    pattern = "^https://.*",
    required = true,
    example = "https://api.example.com/webhooks/products"
)
