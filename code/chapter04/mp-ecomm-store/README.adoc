...
This project is a MicroProfile-based e-commerce application developed using Jakarta EE 10 and MicroProfile 7.1. It demonstrates modern Java enterprise development practices including REST API design, loose coupling, dependency injection, and unit testing strategies.
...
* *MicroProfile 7.1*: Microservices specifications
...
== Testing Async Webhooks with Smee.io

The application includes an asynchronous processing endpoint (`/api/products/async-process`) that demonstrates webhook-style callbacks using MicroProfile OpenAPI 4.1 callback annotations. This section explains how to test this feature using Smee.io, a webhook payload delivery service.

=== What is Smee.io?

https://smee.io[Smee.io] is a free service that allows you to test webhooks and receive HTTP callbacks on your local development machine without exposing your server to the internet. It creates a unique URL that captures webhook payloads and forwards them to your local server.

=== Prerequisites

* Node.js installed on your system
* Application running on `http://localhost:5050/mp-ecomm-store`

=== Step 1: Set Up Smee.io Channel

1. Open https://smee.io in your browser
2. Click btn:[Start a new channel]
3. Copy the generated webhook URL (e.g., `https://smee.io/abc123xyz`)

=== Step 2: Install Smee Client

Install the Smee client globally using npm:
...
Start the Smee client to forward webhook payloads to your local callback endpoint:

[source,bash]
----
smee --url https://smee.io/YOUR_CHANNEL_ID --target http://localhost:3000/callback
----

*Note:* Replace `YOUR_CHANNEL_ID` with your actual Smee.io channel ID.

For this example, you'll need to create a simple callback server on port 3000. Here's a minimal Node.js script:
...
Install Express and start the callback server:
...
In another terminal, start your MicroProfile application:
...
=== Step 5: Send Async Process Request

Send a POST request to the async endpoint with your Smee.io URL as the callback URL:

[source,bash]
----
curl -X POST http://localhost:5050/mp-ecomm-store/api/products/async-process \
  -H "Content-Type: application/json" \
  -d '{
    "product": {
      "id": 5,
    ...
    "callbackUrl": "https://smee.io/YOUR_CHANNEL_ID"
  }'
----
...
=== Step 6: Monitor the Callback

After sending the request:

1. Check the Smee.io web interface - you'll see the incoming webhook payload
2. Check your callback server terminal - you'll see the logged callback data
3. The callback payload follows the `ProcessResult` schema:
...
[source,text]
----
1. Client sends POST to /api/products/async-process with callbackUrl
   ↓
2. Server accepts request (HTTP 202) and initiates processing
   ↓
3. When processing completes, server POSTs result to callbackUrl
   ↓
4. Smee.io receives the POST and forwards to local callback server
   ↓
5. Local callback server receives ProcessResult payload
----

=== OpenAPI Callback Documentation

The async endpoint uses MicroProfile OpenAPI 4.1 callback annotations to document the webhook behavior. You can view the complete API documentation at:

* OpenAPI YAML: http://localhost:5050/mp-ecomm-store/openapi
* Swagger UI: http://localhost:5050/mp-ecomm-store/openapi/ui

The callback is documented with:

* `@Callback` annotation specifying the callback name and URL expression
* `@CallbackOperation` defining the HTTP method and request body schema
* `callbackUrlExpression` using JSONPath: `{$request.body#/callbackUrl}`

=== Troubleshooting

==== Smee Client Connection Issues

If the Smee client can't connect:
...
smee --url https://smee.io/YOUR_CHANNEL_ID --target http://localhost:4000/callback
----

==== Callback Not Received

* Verify your callback server is running and listening on the correct port
* Check the Smee.io web interface to confirm webhooks are being received
* Ensure the `callbackUrl` in your request matches your Smee.io channel URL
* Check server logs for any errors during callback processing

==== Invalid Callback URL

The API expects a valid HTTPS URL. Smee.io provides HTTPS URLs automatically, so this should work correctly. If using a different webhook service, ensure it provides HTTPS.

=== Testing Without Smee.io

For local testing without external services, you can:

1. Use a tool like https://webhook.site[webhook.site] (similar to Smee.io)
2. Deploy a simple callback receiver to a cloud service (Heroku, Railway, etc.)
3. Use `ngrok` to expose your local callback server:

[source,bash]
----
ngrok http 3000
# Use the generated HTTPS URL as your callbackUrl
----
...
* Implement actual async processing (e.g., using JMS, MicroProfile Reactive Messaging, or ExecutorService)
* Add authentication/authorization for callback endpoints
* Implement retry logic for failed callback deliveries
* Store callback URLs securely and validate them before use
* Add rate limiting to prevent abuse
* Log all callback attempts for audit purposes
* Consider using a message queue for reliable webhook delivery
