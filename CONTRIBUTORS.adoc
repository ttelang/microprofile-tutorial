...
The following configuration **must be identical across all branches** (`main`, `6.1`, `v7.1`, etc.):
...
content:
  sources:
    - url: https://github.com/microprofile/microprofile-tutorial.git
      branches: [6.1, v7.1]  # List all version branches here
      start_path: /
----

**Important**: This configuration should be identical on every branch. Even when working on the `6.1` branch, the `antora-assembler.yml` file should still reference both `6.1` and `v7.1` branches.
...
name: microprofile-tutorial
version: 7.1
edit_url: https://github.com/microprofile/microprofile-tutorial/tree/v7.1/modules/ROOT/pages/{path}
----
...
[source,yaml]
----
branches: [6.1, v7.1, v8.0]  # Add the new version
----
