...
In this tutorial, you will learn how to design and build a real-world cloud-native microservices-based e-commerce application using MicroProfile and Jakarta EE. The tutorial will cover using MicroProfile APIs such as Config, REST Client, JWT, Fault Tolerance, Metrics, Telemetry, GraphQL and Reactive Messaging to build efficient, scalable, and resilient microservices for cloud-native applications. We aim to provide a comprehensive overview and hands-on knowledge about using MicroProfile APIs.
...
In this tutorial, you will learn to design and build microservices-based e-commerce application called "MicroProfile e-Commerce store". It would include multiple microservices, each serving a different purpose and highlighting different aspects of MicroProfile. The app will demonstrate how multiple microservices communicate through a combination of REST APIs and event-driven messaging.

A cloud-native e-commerce system is composed of independently deployable microservices that communicate through a combination of REST APIs and event-driven messaging. We will also introduce modern enhancements such as Graph APIs and Reactive Messaging. These improvements reflect how production systems evolve to support scalability, flexibility, and performance at scale. 

At a high level, the architecture uses:

- Synchronous REST communication for user-driven operations
- GraphQL communication for flexible, client-driven data retrieval
- Event-driven architecture using asynchronous messaging for decoupled, scalable processing

This hybrid approach ensures that you achieve both responsiveness and fault tolerance. Developers can use this adaptation as a practical case study to implement MicroProfile APIs in real-world applications.

The MicroProfile e-Commerce store application comprises of the following microservices:

- **Product Catalog**: This service acts as the central repository for all product-related information, including detailed descriptions, and pricing, and inventory levels. It provides APIs for fetching product details efficiently for other microservices, such as the Shopping Cart. This microservice is vital for updating product data, ensuring data consistency and accuracy across the e-Commerce platform.

Responsibilities:

- Manage product details, pricing, and metadata
- Provide APIs for product queries

While the service initially exposes REST endpoints, you will enhance it to support Graph-based APIs using MicroProfile GraphQL.

This enhancement enables:

- Client-driven queries
- Reduced over-fetching and under-fetching
- Aggregation of related data (product, pricing, inventory) in a single request

In modern architectures, REST and Graph APIs often coexist. Use REST for simple operations and Graph APIs for complex data retrieval scenarios.
...
- Manage cart lifecycle (add, remove, update items)
- Calculate totals and discounts
- Initiate order creation

Order creation is initiated through a synchronous REST call from the Shopping Cart Service to the Order Processing Service. This ensures immediate user feedback and validation before order creation.
...
- **Order Processing**: This service manages the entire order process, from collecting shipping information and confirming order details to initiating payment processing. This microservice ensures a seamless transition from shopping to order completion. This service orchestrates the order lifecycle and acts as the bridge between synchronous and asynchronous workflows.

Responsibilities:

- Create and manage orders
- Validate order details and inventory
- Orchestrate payment and shipping processes

Once an order is created via a synchronous REST call, the service transitions into an event-driven model.

Flow: 

1. Shopping Cart triggers Order Service via REST
2. Order Service creates order and publishes OrderCreated event
3. Payment Service processes payment
4. Inventory Service updates stock
5. Shipping Service prepares fulfillment

This approach ensures loose coupling, independent scalability, and failure isolation.
...
These events are emitted using MicroProfile Reactive Messaging.
...
Each microservice owns its data and communicates via events.
