...
Version 4.1 requires that all collection-returning methods in model interfaces return mutable collections. This ensures consistent behavior when modifying the OpenAPI model programmatically:
...
Prior to version 4.1, implementations could return immutable collections, which would cause runtime errors when attempting modifications.
