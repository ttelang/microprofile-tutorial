...
MicroProfile OpenAPI processes documentation from multiple sources and merges them in a specific order. Understanding this order helps you predict how different documentation mechanisms interact and override each other.

The specification mandates the following processing order:

1. **Configuration values**: The implementation reads all `mp.openapi.*` configuration properties.

2. **Model reader**: If configured via `mp.openapi.model.reader`, the OASModelReader creates an initial OpenAPI model tree.

3. **Static OpenAPI file**: The implementation loads any static OpenAPI document from `META-INF/openapi.{yml,yaml,json}` and merges it with the model from step 2.

4. **Annotations**: The implementation scans Jakarta REST resources for OpenAPI annotations and merges them with the model from step 3.

5. **Filter**: If configured via `mp.openapi.filter`, the OASFilter receives callbacks for each element in the model tree and can modify or remove elements.

Each step adds to or overrides the OpenAPI model built in previous steps. For example:

* Annotations can override elements defined in static files
* Filters can modify or remove any elements from the final model
* Configuration properties (like `mp.openapi.servers`) override any server definitions from other sources
...
| `@RequestBody`
| Describes the request body of an HTTP request, specifying the content of the body and whether it is required. In MicroProfile OpenAPI 4.0 and later, request bodies are required by default (`required=true`). To make a request body optional, explicitly set `required=false`.
...
MicroProfile OpenAPI provides several configuration properties to control its behavior. These properties are configured via MicroProfile Config, typically in `microprofile-config.properties`.
...
The following table lists the core configuration properties:
...
| Fully qualified name of the OASModelReader implementation class
| None

| `mp.openapi.filter`
| Fully qualified name of the OASFilter implementation class
| None
...
| Comma-separated list of packages to scan (including subpackages)
| All packages

| `mp.openapi.scan.classes`
| Comma-separated list of specific classes to scan
| All classes

| `mp.openapi.scan.exclude.packages`
| Comma-separated list of packages to exclude from scanning
| None

| `mp.openapi.scan.exclude.classes`
| Comma-separated list of classes to exclude from scanning
| None

| `mp.openapi.scan.beanvalidation`
| Enable Jakarta Bean Validation annotation processing
| `true`

| `mp.openapi.servers`
| Comma-separated list of global server URLs
| None

| `mp.openapi.servers.path.{path}`
| Alternative servers for a specific path
| None

| `mp.openapi.servers.operation.{operationId}`
| Alternative servers for a specific operation (requires `operationId`)
| None

| `mp.openapi.schema.{className}`
| JSON-formatted schema definition for a specific class
| None
...
*Override servers for a specific path:*

[source, properties]
----
mp.openapi.servers.path./products=https://products-api.example.com/v1
----
...
[source, properties]
----
mp.openapi.schema.java.util.Date={"name":"EpochMillis","type":"number","format":"int64","description":"Milliseconds since January 1, 1970, 00:00:00 GMT"}
----

== Features in MicroProfile OpenAPI 4.x

This chapter documents features introduced in MicroProfile OpenAPI 4.0 and refined in 4.1.

[NOTE]
====
**Version history:**

* **MicroProfile OpenAPI 4.0** (March 2024): Major release introducing OpenAPI v3.1 support, webhooks via `@PathItem`, and new schema properties for JSON Schema 2020-12 compliance.

* **MicroProfile OpenAPI 4.1** (March 2025): Clarification release that added:
  ...
  - New convenience methods: `hasExtension()` and `getExtension()`

Unless otherwise noted, this chapter covers features available in both 4.0 and 4.1.
====
...
MicroProfile OpenAPI 4.1 supports both approaches for backward compatibility:

* Use `@Schema(nullable = true)` in your Java code (converted to type arrays in the generated spec)
* Or directly use type arrays via `@Schema(type = {SchemaType.STRING, SchemaType.NULL})`
...
* The filter is called once for each filterable element in the OpenAPI model tree
* **Descendant elements are always filtered before their ancestors** (bottom-up processing)
* The `filterOpenAPI` method is **always called last**, after all other elements have been filtered
* The order in which sibling elements are filtered is undefined

This bottom-up approach ensures child elements are fully processed before their parents, allowing parent filters to make decisions based on their children's final state.
====
...
MicroProfile OpenAPI automatically processes Jakarta Bean Validation annotations and uses them to enhance the generated OpenAPI schemas with validation constraints. This integration eliminates the need to duplicate validation rules in both your code and OpenAPI documentation.

=== Supported validation annotations

The following table shows which Jakarta Bean Validation annotations affect OpenAPI schemas:

[cols="1,1,2", options="header"]
|===
| Annotation | Schema Type | Generated Properties
...
| `@NotBlank`
| `string`
| `pattern: \S` (requires non-whitespace)
...
| `@Size(min=a, max=b)`
| `array`
| `minItems: a`, `maxItems: b`

| `@Size(min=a, max=b)`
| `object`
| `minProperties: a`, `maxProperties: b`

| `@DecimalMax(value=a)`
| `number` or `integer`
| `maximum: a`

| `@DecimalMax(value=a, inclusive=false)`
| `number` or `integer`
| `exclusiveMaximum: a`
...
| `@DecimalMin(value=a, inclusive=false)`
| `number` or `integer`
| `exclusiveMinimum: a`
...
=== Controlling Bean Validation processing

Bean Validation annotation processing is enabled by default. You can disable it using the configuration property:
...
[NOTE]
====
Bean Validation annotations only affect the schema when:

* The annotation is applied to an element for which a schema is generated
* The annotation's `groups` attribute is empty or includes `jakarta.validation.groups.Default`
* The property hasn't been explicitly set using `@Schema` annotations
====
...
[NOTE]
====
This section demonstrates a webhook subscription pattern using operation callbacks (`@Callback`). This is different from OpenAPI v3.1's webhooks feature, which documents callbacks configured through out-of-band mechanisms. MicroProfile OpenAPI supports operation callbacks via the `@Callback` annotation. For documenting OpenAPI v3.1 webhooks, you can use the `webhooks` property in the `@OpenAPIDefinition` annotation (added in MicroProfile OpenAPI 4.0) or static OpenAPI files.
====
...
=== Documenting webhooks with @PathItem

MicroProfile OpenAPI 4.0 added support for documenting webhooks directly using the `webhooks` property in `@OpenAPIDefinition`. Webhooks represent out-of-band callbacks that your API will call, but unlike operation callbacks, they are not directly tied to a specific API operation.
...
@OpenAPIDefinition(
    info = @Info(
        ...
        version = "1.0.0",
        description = "API with webhook notifications"
    ),
    webhooks = {
        @PathItem(
            name = "productCreated",
            description = "Webhook sent when a new product is created",
            post = @PathItemOperation(
                summary = "Product Created Event",
                description = "Sent to registered webhook URLs when a product is created",
                requestBody = @RequestBody(
                    description = "Product creation event data",
                    content = @Content(
                        mediaType = "application/json",
                        schema = @Schema(implementation = ProductEvent.class)
                    )
        ...
        @PathItem(
            name = "productUpdated",
            description = "Webhook sent when a product is updated",
            post = @PathItemOperation(
                summary = "Product Updated Event",
                description = "Sent to registered webhook URLs when a product is updated",
                requestBody = @RequestBody(
                    description = "Product update event data",
                    content = @Content(
                        mediaType = "application/json",
                        schema = @Schema(implementation = ProductEvent.class)
                    )
...
This generates a `webhooks` section in the OpenAPI specification:
...
[NOTE]
====
The difference between webhooks and callbacks:

* **Callbacks** (`@Callback`): Documented on specific API operations. The callback URL is provided as part of the request to that operation.

* **Webhooks** (`@PathItem` in `@OpenAPIDefinition.webhooks`): Documented at the API level. Webhook URLs are configured through a separate registration mechanism (not shown in the OpenAPI spec).
====
...
===== Understanding security requirement annotations

MicroProfile OpenAPI provides three annotations for documenting security requirements:

* **`@SecurityRequirement`**: Documents a single security scheme requirement. Use this when an operation requires one specific authentication method.

* **`@SecurityRequirements`**: Documents alternative security schemes where **any one** can satisfy the requirement (OR logic). Use this when clients can choose between different authentication methods.

* **`@SecurityRequirementsSet`** (added in MicroProfile OpenAPI 3.1): Documents multiple security schemes that **all must** be satisfied (AND logic). Use this when an operation requires multiple authentication methods simultaneously.

*Example with single requirement:*
...
*Example with alternative requirements (OR logic):*
...
// Client can authenticate with EITHER apiKey OR bearerAuth
@GET
@Path("/public-data")
...
*Example with multiple requirements (AND logic):*
...
// Client MUST provide BOTH apiKey AND bearerAuth
@DELETE
@Path("/admin/{id}")
...
The generated OpenAPI specification reflects these requirements:
...
paths:
  ...
  /products/public-data:
    get:
      # Alternative requirements (OR) - client can use either
      security:
        - apiKey: []
        - bearerAuth: []
  ...
  /products/admin/{id}:
    delete:
      # Multiple requirements (AND) - client must use both
      security:
        - apiKey: []
          bearerAuth: []
----
