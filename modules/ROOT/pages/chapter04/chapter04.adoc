    ...
    CategoryRecord:
      ...
      properties:
        ...
        displayOrder:
          ...
          - 1
----
Use `@NotNull` validation annotations or `@Schema(required = true)` to explicitly mark components as required in the generated specification. In this example, `@NotNull` annotations mark `id`, `name`, and `displayOrder` as required, while `parentId` remains optional for root categories.

==== Optional Components

If you need nullable components, use `Optional<T>`:
...
public record CategoryRecord(
    ...
    Optional<Long> parentId,  // Optional for root categories
...
*Generated schema with optional parentId:*
[source, yaml]
----
openapi: 3.1.0
components:
...
Use `@NotNull` or `@Schema(required = true)` to mark required components. For nullable components, use `@Schema(nullable = true)` or wrap the type in `Optional<T>`. In this example, `id`, `name`, and `displayOrder` are required, while `parentId` is nullable for root categories.
