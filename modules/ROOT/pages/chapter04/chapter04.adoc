...
== Documenting API features

This section covers how to document common API features using MicroProfile OpenAPI annotations.
...
*Applying security to an entire resource class:*

[source, java]
----
package io.microprofile.tutorial.store.admin.resource;
...
@Path("/admin/products")
@SecurityRequirement(name = "oauth2", scopes = {"write:products", "delete:products"})
public class AdminProductResource {
    
    // All operations in this class require OAuth2 with both scopes
    
    @PUT
    @Path("/{id}/publish")
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(summary = "Publish product")
    public Response publishProduct(@PathParam("id") Long id) {
    ...
    public Response bulkImport(List<Product> products) {
