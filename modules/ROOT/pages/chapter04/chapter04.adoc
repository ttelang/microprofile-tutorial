...
MicroProfile OpenAPI supports documenting asynchronous operations that use callbacks and webhooks. The `@Callback` annotation documents a callback that can be set up using an operation. Support for webhooks was added in MicroProfile Open API 4.0, which document a callback that can be set up via some out-of-band mechanism (e.g., through a form on a web page). The OpenAPI document may include information on how to set the webhook up.
