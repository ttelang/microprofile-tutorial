...
==== Example 1: Async Product Processing
...
public class ProductResource {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(summary = "Process product updates asynchronously")
    ...
    public Response processProduct(AsyncProductRequest request) {
...
class AsyncProductRequest {
    @Schema(required = true, example = "https://client.example.com/webhook")
...
@Schema(description = "Processing result sent to callback URL")
class ProcessResult {
    ...
    @Schema(example = "Product successfully created")
...
* `@Callback(name = "processComplete")`: Names this callback operation
* `callbackUrlExpression = "{$request.body#/callbackUrl}"`: Uses JSON Pointer to extract callback URL from request
* `@CallbackOperation`: Documents the HTTP POST your API will make to the callback URL
* `@RequestBody`: Describes the JSON payload sent to the callback

This generates OpenAPI documentation showing:
...
paths:
  /products/async:
    post:
      summary: Process product updates asynchronously
      callbacks:
        processComplete:
...
==== Example 2: Webhook Event Subscriptions

For more complex scenarios with multiple event types, you can document multiple callback operations. This example shows a webhook subscription system with five different event types:
    ...
    @Callback(
        ...
        operations = {
            ...
            @CallbackOperation(
                ...
                description = "Triggered when an existing product is modified via PUT /products/{id}",
                requestBody = @RequestBody(
            ...
            @CallbackOperation(
                ...
                description = "Triggered when a product is removed via DELETE /products/{id}",
                requestBody = @RequestBody(
...
*WebhookSubscription* - Subscription configuration sent by clients:
...
*ProductEvent* - Event payload sent to webhook subscribers:
...
The callbacks are documented in the OpenAPI specification under the `callbacks` section:
...
==== Understanding Callback Annotations
...
|`name`
|Identifier for this callback group (e.g., "productEvents")

|`callbackUrlExpression`
|JSON Pointer expression to extract callback URL from request body. +
Format: `{$request.body#/fieldName}` +
Example: `{$request.body#/callbackUrl}` extracts the `callbackUrl` field

|`operations`
|Array of `@CallbackOperation` annotations defining each webhook event type
|===

*@CallbackOperation Annotation Parameters:*
...
|`method`
|HTTP method for the callback (typically "post")

|`summary`
|Brief description of when this callback is triggered

|`description`
|Detailed explanation including headers sent, retry policy, etc.

|`requestBody`
|Schema and examples of the webhook payload

|`responses`
|Expected responses from the client's webhook endpoint
|===
...
Documenting webhooks in OpenAPI provides significant advantages:

* *Clear Integration Contract*: Developers know exactly what events are available and what data to expect
* *Interactive Documentation*: Swagger UI displays all webhook events with examples
* *Client Code Generation*: Tools can generate webhook handler stubs automatically
* *Testing*: Mock servers can simulate webhook deliveries for integration tests
* *Validation*: Schema validation ensures webhook payloads match the specification
* *Discoverability*: All webhook events are documented in one place
* *Maintenance*: Changes to webhook schemas are tracked and versioned
...
IMPORTANT: The security annotations in this section are for *documentation purposes only*. They describe to API consumers how to authenticate with your API but do not implement or enforce any security controls. You must implement actual authentication and authorization using Jakarta Security, JAX-RS filters, or other security mechanisms before documenting them with OpenAPI annotations.
...
IMPORTANT: OpenAPI annotations *document* security requirements but do *not enforce* them. Before using these annotations, you must first implement actual access controls in your application (using Jakarta Security, JAX-RS security annotations, filters, or interceptors). Once your security is implemented, use OpenAPI annotations to document how clients should authenticate with your API.
...
You discovered how explicit `@Target` meta-annotations improve compile-time checking and IDE support, helping you use OpenAPI annotations correctly and preventing misuse during development.
