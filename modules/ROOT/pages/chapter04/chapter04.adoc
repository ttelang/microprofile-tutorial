...
- Documenting Asyncronous operations with callbacks
...
public class CustomModelReader implements OASModelReader {
    ...
    public OpenAPI buildModel() {
        return OASFactory.createOpenAPI()
            ...
            // Alternative: Use pure JSON Schema 2020-12. Use this ONLY if you need strict compatibility
...
public class ProductResource {
    ...
    @Callback(
        ...
        operations = {
            @CallbackOperation(
                method = "post",
                summary = "Product upate processing completed",
...
The callback is documented in the OpenAPI documentation as below:
...
For more complex scenarios with multiple event types, you can use webhooks. This example shows a webhook subscription system with five different event types:
...
=== Benefits of Documenting Callbacks and Webhooks

Documenting webhooks and callbacks in OpenAPI specification provides the following advantages:

* *Discoverability*: All webhook events are documented in one place
* *Maintenance*: Changes to webhook schemas can be tracked and versioned
* *Clear contract*: Developers know exactly what events are available and what data to expect. This also serves as developer's integration guide.
* *Interactive Documentation*: Swagger UI displays all webhook events with examples 
* *Testing support*: Tools can generate mock callback endpoints and these mock servers can simulate webhook deliveries for integration tests 
* *Code generation*: Tools can generate callback handler code for clients and webhook handler stubs automatically
