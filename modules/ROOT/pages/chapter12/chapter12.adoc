...
GraphQL is a modern query language and runtime for APIs that provides a powerful alternative to traditional REST APIs. This chapter explores MicroProfile GraphQL 2.0, a specification that brings GraphQL capabilities to Jakarta EE and MicroProfile applications. Use the patterns and APIs described here to build flexible, efficient GraphQL APIs that allow clients to request exactly the data they need.
...
- What's New in MicroProfile GraphQL 2.0
...
*GraphQL*: Caching is more complex due to the flexible nature of queries but can be implemented at the application level or using specialised tools.
...
# Updated for MicroProfile GraphQL 2.0
...
mp.graphql.showErrorMessage=com.example.BusinessException
----

The `mp.graphql.hideErrorMessage` property specifies a comma-separated list of fully qualified exception class names whose error messages are replaced with the default error message when returned to clients. By default, all unchecked exceptions (subclasses of `RuntimeException`) have their messages hidden. The `mp.graphql.showErrorMessage` property specifies exception class names whose error messages are sent to clients. By default, all checked exceptions have their messages shown.
...
You can also use dynamic configuration to adjust behaviour at runtime:
...
=== Optimising Complex Nested Queries Using @Batch
...
public class OrderApi {
    ...
    public CompletionStage<List<Product>> product(@Source List<OrderItem> items) {
...
*4. Optimise Database Queries*:

[source, java]
----
@ApplicationScoped
public class OptimisedProductService {
...
* Optimising performance with caching, batching, and efficient query patterns
...
MicroProfile GraphQL 2.0 provides a robust foundation for building modern APIs that give clients the flexibility to request exactly the data they need while maintaining strong typing and excellent developer experience.
