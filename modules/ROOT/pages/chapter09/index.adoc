...
This chapter explores the fundamentals of MicroProfile Telemetry 2.1, covering topics such as observability in microservices, tracing concepts, metrics collection, instrumenting Telemetry, setting up tracing providers, context propagation and correlation, integrating with other MicroProfile specifications, analysing traces, deployment, visualisation, security considerations for tracing, and more. By the end of this chapter, readers will learn how to effectively leverage distributed tracing and metrics for debugging, performance monitoring, and system optimisation.
...
** Default Behaviour and Auto-Discovery
...
* *Metrics*: Provide quantitative measurements of system behaviour, such as request counts, response times, and resource utilisation.
...
* *Complexity due to Distributed Architecture*: Microservices are often deployed across multiple nodes, containers, or cloud environments, making it challenging to track requests as they move through the system. This lack of visibility increases debugging complexity, making it harder to identify bottlenecks and analyse system behaviour.
...
To address these challenges, MicroProfile Telemetry 2.1 specification provides a standardized set of APIs for capturing telemetry data, including trace information, metrics, and context propagation, to improve observability in distributed systems. By enabling seamless tracing and metrics collection, developers can analyse system behaviour, troubleshoot service interactions, and ensure application reliability. 
...
Together, these concepts form the foundation of distributed tracing, enabling developers to monitor, analyse, and optimise the performance of their microservices effectively.

=== Metrics and Instruments

MicroProfile Telemetry 2.1 provides comprehensive support for metrics through the OpenTelemetry Metrics API. Metrics provide quantitative measurements of system behaviour and are essential for monitoring application performance, resource utilisation, and business-level indicators.
...
* Monitor resource utilisation (CPU, memory, network)
* Measure business-specific indicators (orders processed, revenue generated)
* Identify trends and anomalies in system behaviour
...
Alternatively, it is possible to use the MicroProfile Platform BOM (Bill of Materials) to manage all MicroProfile dependencies:
...
The MicroProfile Platform BOM can also be used with Gradle:
...
If direct access to OpenTelemetry APIs for advanced use cases is required, it is also possible to add the OpenTelemetry dependencies:
...
The span name can be customized by providing a value to the annotation:
...
`@WithSpan` can be used for tracing key business operations, such as order processing, payment handling, or API requests.
...
Attributes enhance trace context by attaching key-value pairs to a span, providing additional metadata that helps filter and analyse traces in observability tools:
...
Parent-child relationships can also be explicitly created when using the Tracer API:
...
Configuration values can be injected into beans:
...
Telemetry-related headers can be documented in the OpenAPI specification:
...
[source,properties]
----
# Tracing for health endpoints can be disabled if desired
...
For development and debugging, the console exporter can be used:
...
public class ContextManipulationService {
    ...
    public void explicitContextManagement() {
        ...
        try (Scope scope = contextWithSpan.makeCurrent()) {
            ...
            // Contexts can also be temporarily switched
            detachedWork();
...
This section covers tools for analysing traces and metrics, and how to visualise telemetry data.
...
This setup ensures that each payment transaction is fully traceable, allowing developers to monitor execution flow, debug issues, and optimise application performance effectively.
...
Attributes enhance trace context by attaching key-value pairs to a span, providing additional metadata that helps filter and analyse traces in observability tools. This helps in contextualizing the trace data:
...
The following tools are commonly used for trace collection, visualisation, and analysis in MicroProfile applications:
...
https://www.jaegertracing.io/[Jaeger] is an open-source distributed tracing system developed by Uber, widely used for monitoring microservices and visualising request flows in cloud-native applications. It provides a powerful visualisation interface that enables developers to inspect traces, analyse dependencies between services, and examine execution timelines, making it an essential tool for debugging performance bottlenecks.
...
https://zipkin.io/[Zipkin] is a distributed tracing system designed to help developers visualise and diagnose latency issues in microservices-based applications. It provides a lightweight and fast tracing solution, making it ideal for quick deployment with minimal resource usage. Its simplicity and efficiency make it a popular choice for teams looking to implement tracing without significant infrastructure overhead.
...
When implementing tracing in applications, it is crucial to be mindful of security implications. Tracing involves collecting and storing data about application behaviour, which can potentially expose sensitive information if not handled properly.
...
By following these security considerations, developers can leverage the benefits of distributed tracing without compromising the security of the system or the privacy of users. Careful handling of trace data, coupled with robust encryption, access controls, and compliance practices, ensures that tracing remains a valuable yet secure component of the observability strategy.

== Conclusion

MicroProfile Telemetry 2.1 provides a robust and modernized foundation for observability in Jakarta EE and MicroProfile-based microservices. By leveraging the latest OpenTelemetry 1.48.0 capabilities and Jakarta EE 10 integration, developers can implement comprehensive distributed tracing and metrics collection with simplified configuration and enhanced features.

Throughout this chapter, we explored the key capabilities of MicroProfile Telemetry 2.1:

* *Core Observability Concepts*: Understanding spans, traces, context propagation, and the three pillars of observability (traces, metrics, and logs)
* *Automatic and Manual Instrumentation*: Leveraging automatic instrumentation for Jakarta RESTful Web Services while having the flexibility to create custom spans using @WithSpan annotations or the Tracer API
* *Metrics Collection*: Using Counter, Histogram, and Observable Gauge instruments to track application performance and business metrics
* *Seamless Integration*: Working with other MicroProfile specifications including Config, OpenAPI, Health, and Fault Tolerance
* *Flexible Configuration*: Using microprofile.telemetry.* properties to configure exporters, sampling, and other telemetry settings
* *Advanced Features*: Utilizing advanced APIs for context manipulation, baggage propagation, and fine-grained span control
* *Deployment and Visualisation*: Setting up observability backends like Jaeger, Grafana Tempo, and Prometheus to visualise and analyse telemetry data

MicroProfile Telemetry 2.1 brings significant improvements over previous versions, including simplified configuration (no longer requiring special classloader settings), upgraded OpenTelemetry support, and full Jakarta EE 10 compatibility. These enhancements make it easier than ever to add comprehensive observability to your microservices.

The integration of standardized tracing and metrics, combined with vendor-neutral APIs, ensures that developers can switch between different observability platforms without modifying application code. This flexibility is crucial for organizations that need to adapt to changing requirements or integrate with various monitoring solutions.

Security remains a critical consideration when implementing telemetry. By following best practices—sanitizing sensitive data, encrypting trace transmissions, implementing proper access controls, and adhering to compliance regulations—developers can harness the power of observability while protecting user privacy and system security.

As microservices architectures continue to evolve and become more complex, the need for comprehensive observability grows. MicroProfile Telemetry 2.1 stands as an essential tool for modern cloud-native applications, providing developers with the visibility they need to:

* *Debug Complex Issues*: Trace requests across multiple services to identify the root cause of problems
* *Optimise Performance*: Analyse bottlenecks and improve system efficiency
* *Monitor System Health*: Track key metrics and detect anomalies before they impact users
* *Ensure Reliability*: Proactively identify and resolve issues to maintain high availability

By adopting MicroProfile Telemetry 2.1 in your applications, you gain the observability foundation necessary to deliver reliable, high-performance microservices that meet the demanding requirements of modern enterprise environments. The combination of automatic instrumentation, flexible manual control, comprehensive metrics support, and seamless integration with the MicroProfile ecosystem makes it an indispensable tool for building observable, maintainable, and scalable cloud-native applications.
